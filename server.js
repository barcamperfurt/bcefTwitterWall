...
app.twSearch = function(keyword, sock) {
    ...
    for (var i = 0; i < config.app.hashtags.length; i++) {
        mtwit.get('search/tweets', { q: config.app.hashtags[i] }, function(
            err,
            data
        ) {
            if (!err && data && data.statuses) {
                ...
                for (var tweet in data.statuses) {
                    ...
                    if (data.statuses[tweet].CHECK.tweetisok) {
                        data.statuses[tweet].text = utils.parseHasTags(
                            data.statuses[tweet].text
                        );
                        data.statuses[tweet].text = utils.parseLinks(
                            data.statuses[tweet].text
                        );
...
app.twStream = function(sock) {
    if (!app.twitterInitialized) {
        twit.stream('statuses/filter', { track: config.app.hashtags }, function(
            stream
        ) {
